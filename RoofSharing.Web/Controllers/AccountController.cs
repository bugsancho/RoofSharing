...
namespace RoofSharing.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            {
                firstName = firstNameFbClaim.Value;
            }
            var lastNameFbClaim = loginInfo.ExternalIdentity.Claims.FirstOrDefault(x => x.Type == "urn:facebook:last_name");
            if (lastNameFbClaim != null)
            {
                lastName = lastNameFbClaim.Value;
            }
            var fbUserId = loginInfo.ExternalIdentity.Claims.FirstOrDefault(x => x.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier");
            if (fbUserId != null)
            {
                profilePicUrl = "https://graph.facebook.com/" + fbUserId.Value + "/picture";
            }
            ...
            switch (result)
            {
                ...
                default:
                    ...
                    return View("ExternalLoginConfirmation", new ExternalLoginConfirmationViewModel { Email = loginInfo.Email, FirstName = firstName, LastName = lastName, PictureUrl = profilePicUrl });
            }
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, HttpPostedFileBase uploadedPicture, string returnUrl)
        {
